...
const ChunkStatus ChunkStatus::Surface = create(/*5*/1, 0, [](ServerWorld* world, WorldLightManager& lightManager, ChunkGenerator& generator, int32_t x, int32_t z, Chunk& chunk, std::span<Chunk*> chunks, int64_t seed) {
...
const ChunkStatus ChunkStatus::Features = create(/*8*/2, 1, [](ServerWorld* world, WorldLightManager& lightManager, ChunkGenerator& generator, int32_t x, int32_t z, Chunk& chunk, std::span<Chunk*> chunks, int64_t seed) {
    WorldGenRegion region{world, chunks, 1, x, z, seed};
    generator.generateFeatures(region, chunk);
});

const ChunkStatus ChunkStatus::Light = create(/*9*/3, 1, [](ServerWorld* world, WorldLightManager& lightManager, ChunkGenerator& generator, int32_t x, int32_t z, Chunk& chunk, std::span<Chunk*> chunks, int64_t seed) {
    WorldGenRegion region{world, chunks, 1, x, z, seed};
    lightManager.calculate(region, x << 4, z << 4);
});

const ChunkStatus ChunkStatus::Full = create(/*10*/4, 0, [](ServerWorld* world, WorldLightManager& lightManager, ChunkGenerator& generator, int32_t x, int32_t z, Chunk& chunk, std::span<Chunk*> chunks, int64_t seed) {
