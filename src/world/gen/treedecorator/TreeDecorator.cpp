...
void TreeDecorator::placeVine(WorldWriter &writer, const BlockPos &pos, /*BooleanProperty property,*/ std::set<BlockPos> &positions, BoundingBox &boundingBox) {
    placeBlock(writer, pos, Blocks::VINE->getDefaultState()/*.with(property, true)*/, positions, boundingBox);
}
