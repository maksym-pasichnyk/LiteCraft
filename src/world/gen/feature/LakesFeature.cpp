    ...
    for (int k1 = 0; k1 < 16; ++k1) {
        for (int l2 = 0; l2 < 16; ++l2) {
            for (int k = 0; k < 8; ++k) {
                ...
                if (flag) {
                    auto material1 = reader.getData(pos.add(k1, k, l2)).getMaterial();
                    if (k >= 4 && material1->isLiquid) {
                        return false;
                    }

                    if (k < 4 && !material1->isSolid && reader.getData(pos.add(k1, k, l2)) != cfg.state) {
                        return false;
                    }
                }
            }
        ...
        for (int i3 = 0; i3 < 16; ++i3) {
            for (int i4 = 0; i4 < 8; ++i4) {
                if (flags[(l1 * 16 + i3) * 8 + i4]) {
                    reader.setData(pos.add(l1, i4, i3), i4 >= 4 ? AIR : cfg.state/*, 2*/);
                }
            }
        }
    }
        ...
        for (int j3 = 0; j3 < 16; ++j3) {
            for (int j4 = 4; j4 < 8; ++j4) {
                if (flags[(i2 * 16 + j3) * 8 + j4]) {
                    const auto blockpos = pos.add(i2, j4 - 1, j3);
                    if (isDirt(reader.getData(blockpos).getBlock())/* && reader.getLightFor(LightType.SKY, pos.add(i2, j4, j3)) > 0*/) {
                ...
                for (int k4 = 0; k4 < 8; ++k4) {
                    ...
                    if (flag) {
                        if ((k4 < 4 || random.nextInt(2) != 0) &&
                            reader.getData(pos.add(j2, k4, k3)).getMaterial()->isSolid) {
                            reader.setData(pos.add(j2, k4, k3), Blocks::STONE->getDefaultState()/*, 2*/);
                        }
                    }
                }
            }
    ...
    if (material == Materials::WATER) {
        for (int k2 = 0; k2 < 16; ++k2) {
            for (int l3 = 0; l3 < 16; ++l3) {
                const auto blockpos1 = pos.add(k2, 4, l3);
