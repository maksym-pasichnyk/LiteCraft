...
static auto joinIsNotEmpty(std::span<const Shape> shapes, const Shape& aabb) -> bool {
    auto it = std::find_if(
        shapes.begin(),
        shapes.end(),
        [&aabb](const auto& element) -> bool {
            return aabb.intersect(element);
        }
    );
    return it != shapes.end();
}

void VillageStructure::createComponents(StructurePieces &pieces, StructureGenerateContext &context, const StructureConfig &config) {
            ...
            for (auto element : elements) {
                ...
                for (auto new_rotation : rotations) {
                    for (auto&& info_b : element->getJigsawBlocks(context.templateManager, BlockPos{}, new_rotation, random)) {
                        ...
                        auto old_bounds = node.piece->shape;
