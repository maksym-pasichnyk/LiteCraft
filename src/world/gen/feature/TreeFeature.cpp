...
bool TreeFeature::generate(WorldGenRegion &reader, ChunkGenerator &generator, Random &random, BlockPos pos, const FeatureConfig &config) {
    ...
    const auto flag = place(reader, random, pos, set, set1, mutableboundingbox, cfg);
...
bool TreeFeature::place(WorldGenRegion &reader, Random &random, const BlockPos &pos, std::set<BlockPos> &set1, std::set<BlockPos> &set2, BoundingBox &boundingBoxIn, const BaseTreeFeatureConfig &config) {
    const auto trunkHeight = config.trunkPlacer->getRandomHeight(random);
    const auto j = config.foliagePlacer->getHeight(random, trunkHeight, config);
    const auto k = trunkHeight - j;
    const auto l = config.foliagePlacer->getRadius(random, k);
    auto blockpos = pos;
    if (!config.forcePlacement) {
        const auto i1 = reader.getHeight(HeightmapType::OCEAN_FLOOR, pos).y;
        const auto j1 = reader.getHeight(HeightmapType::WORLD_SURFACE, pos).y;
    ...
    if (blockpos.y >= 1 && blockpos.y + trunkHeight + 1 <= 256) {
        ...
        const auto l1 = func_241521_a_(reader, trunkHeight, blockpos, config);
...
int TreeFeature::func_241521_a_(WorldGenRegion &reader, int p_241521_2_, const BlockPos &pos, const BaseTreeFeatureConfig &config) {
    for (int i = 0; i <= p_241521_2_ + 1; ++i) {
        const auto j = config.minimumSize.func_230369_a_(p_241521_2_, i);

        for (int k = -j; k <= j; ++k) {
            for (int l = -j; l <= j; ++l) {
                const auto blockpos = pos.add(k, i, l);
